...
#let Title = "山东大学计算机科学与技术学院数据库系统课后作业"
#let author = "arshtyi"
...
#exam-info(info: (
    ...
    课本: link("https://www.cmpedu.com/books/book/5610387.htm", "The book"),
))
...
#question[
    ...
    + 为`"First Bank Corporation"`工作的员工的`ID`、姓名和居住城市。
    + 为`"First Bank Corporation"`工作且收入超$10000$美元的员工的ID、姓名、居住城市。
    + 不为`"First Bank Corporation"`工作的员工的`ID`。
    + 收入超`"Small Bank Corporation"`的每个员工的`ID`。
    + 设公司可能位于几个城市。找到位于`"Small Bank Corporation"`所在所有城市的每个公司名称。
    + 员工最多的公司名称（如果有员工最多的公司有多个，则一并列出）。
    + 员工平均收入高于`"First Bank Corporation"`平均收入的公司名。
    #schema[
...
#question[
    ...
    + 员工的`ID`和姓名，这些员工住在与他们工作的公司所在的城市相同的城市。
    + 员工的`ID`和姓名，这些员工住在与他们的经理所在的城市和街道相同的城市和街道。
    + 员工的`ID`和姓名，这些员工的收入超过他们所在公司的所有员工的平均收入。
    + 拥有最小工资总额的公司名称。
]

#question[
    （_3.17_） 考虑图3-19（亦如@S3）的员工数据库，其中主码下划线。给出以下查询的SQL表达式：
    + 给`"First Bank Corporation"`的所有员工加薪$10%$。
    + 给`"First Bank Corporation"`的所有经理加薪$10%$。
    + 删除`"Small Bank Corporation"`员工在_works_关系中的所有元组。
]

#question[
    （_3.21_） 考虑图3-20（亦如下）的图书馆数据库，其中主码下划线。给出以下查询的SQL表达式：
    + 至少借过一本由`"McGraw-Hill"`出版的书的会员的会员号和姓名。
    + 借过`"McGraw-Hill"`出版的每本书的会员的会员号和姓名。
    + 对于每个出版社，找到借过该出版社的超过五本书的会员的会员号和姓名。
    + 找到会员平均借书数量。考虑到若一个会员没有借过任何书，那么该会员在_borrowed_关系中根本不出现，但该会员仍然计入平均数中。
    #schema[
...
#question[
    （_4.1_） 以下找到$2017$年春季所有课程标题及教师姓名的SQL查询有何问题：
    ```sql
    SELECT name, title
    FROM instructor NATURAL JOIN teaches NATURAL JOIN section NATURAL JOIN course WHERE semester = 'Spring' AND year = 2017;
    ```
]
...
#question[
    （_6.5_） 定义关系代数的*除法算子*：设关系$r(R),s(S)$且$S subset.eq R$。给定一个元组$t$，令$t[S]$表示元组$t$在$S$中属性上的投影。那么，$r div s$是$R-S$上的一个关系。元组$t$在$r div s$的充要条件是：
    - $t$在$product_(R-S)(r)$中
    - 对于每个$s$中的元组$t_s$，在$r$中存在一个元组$t_r$同时满足：
        + $t_r[S] = t_s[S]$
        + $t_r[R-S] = t$
    根据上述定义：
    + 用除法算子写出一个关系代数表达式，找出选修过全部计算机科学课程的学生`ID`。
    + 展示如何在不适用除法的情况下，使用关系代数表达式来表示上述查询。
]

#question[
    （_6.9_） 考虑图6-13（亦如下）的关系数据库，其中主码下划线。为下面查询写出元组关系演算：
    + 找出所有直接为`"Jones"`工作的员工。
    + 找出所有直接为工作的员工居住的城市。
    + 找出`"Jones"`的经理的经理的姓名。
...
#question[
    （_6.13_） 考虑图6-13（亦如@S8）的员工数据库。为下面查询给出元组关系演算表达式：
    + 找出所有为`"FBC"`工作的员工的姓名。
...
#question[
    ...
    + 若需实时查询，选择哪种方式？为什么？
    + 若有一个非常大的客户关系，仅一些磁盘块经常访问，选择哪种方式？为什么？
]
...
#question[
    （_10.4_） 考虑从图10-6（亦如下）中的文件中删除记录$5$。比较下列实现：
...
#question[
    （_12.18_） 考虑关系$r_1(A,B,C),r_2(C,D,E),r_3(E,F)$，主码分别为$A,C,E$。$r_1$有$1000$个元组，$r_2$有$1500$个元组，$r_3$有$750$个元组。估计$r_1 bowtie.big r_2 bowtie.big r_3$的规模并给出一种高效的计算策略。
]

#question[
    （_12.19_） 考虑@1-4-2 中的关系$r_1(A,B,C),r_2(C,D,E),r_3(E,F)$。假设除了整个模式外不存在主码。令$V(C,r_1)=900,V(C,r_2)=1100,V(E,r_2)=50,V(E,r_3)=100$。设$r_1$有$1000$个元组，$r_2$有$1500$个元组，$r_3$有$750$个元组。估计$r_1 bowtie.big r_2 bowtie.big r_3$的规模并给出一种高效的计算策略。
]
...
= No.5
#question[
    （_7.1_） 为车辆保险公司构建一张E-R图：每位客户拥有一辆或多辆车。每辆车关联$0$次或多次事故记录。每张保险单为一辆或多辆车投保并与一次活多次保费支付关联，每次支付仅针对一段特定时间，具有关联到的到期日和缴费日。
]

#question[
    （_7.2_） 考虑如下数据库：包含来自大学模式的_student_、_course_、_section_实体集且记录了学生在不同课程段的不同考试中得分。
    + 使用三元联系作为设计的一部分构建一张E-R图将考试建模为实体。
    + 构建一张可替代的E-R图，仅使用_student_和_section_之间的二元联系。保证在特定的_student_和_section_对间仅存在一个联系且可以表示出学生在不同考试中得分。
]

#question[
    （_7.3_） 设计一张E-R图用于追踪球队得分。应该保存打过的比赛、每场比赛比分、每场比赛上场队员以及每个队员在每场比赛的得分。总的数据应该被建模为派生属性并附上解释以说明如何计算。
]

#question[
    （_7.21_） 考虑图7-30（亦如下）的E-R图，其对一家网上书店建模。
    + 假设书店增加了蓝光光盘和可下载视频。相同的商品可能以多种格式存在，对于不同格式具有不同的价格。绘制E-R图的一部分为这个新增需求建模仅显示与视频相关部分。
    + 现在对完整E-R图扩展从而对包含书、蓝光光盘和可下载视频的任意组合的购物情况进行建模。
    #figure(er-diagram({
        let relation = relation.with(cardinality: ("", ""))
        entity(
            (0, 0),
            label: "author",
            name: "author",
            attributes: (
                west: ("name", "address", "URL"),
            ),
            primary-key: "name",
        )
        entity(
        ...
        relation(
            ...
            name: "basket_of",
            label: "basket_of",
            entities: ("shopping_basket", "customer"),
        )
    }))
]

#question[
    （_7.22_） 为一家汽车公司设计数据库用于给经销商提供协助以维护客户记录和经销商库存并协助销售人员订购。

    每辆车由车辆标识号（Vehicle Identification Number，VIN）标识，每辆单独的车都是公司提供的特定品牌的特定模型（如XF是塔塔汽车捷豹品牌车的模型）。每个模型都可以有各种选项但是一辆车可能只有一些（或没有）可用选项。数据库需要保存关于模型、品牌、选项的信息以及每个经销商、客户、车的消息。

    设计应该包括E-R图、关系模式的集合以及包括主码和外码约束在内的一系列约束。
]

#question[
    （_7.23_） 为全球包裹递送公司（如DHL、FedEX）设计数据库，必须能够追踪寄件客户和收件客户（有些客户可能两者皆是）。每个包裹必须是可标识且可追踪的，因此数据库必须能存储包括的当前位置和历史位置（包括卡车、飞机、机场、仓库）。

    设计应该包含E-R图、关系模式的集合以及包括主码和补码约束在内的一系列约束。
]

= No.6
#question[
    （_8.1_） 设模式$R=(A,B,C,D,E)$分解为$(A,B,C),(A,D,E)$且如下函数依赖集$F$成立：$ A arrow.r B C\ C D arrow.r E \ B arrow.r D\ E arrow.r A $证明该分解为无损分解。
]

#question[
    （_8.3_） 如何使用函数依赖声明：
    + _student_和_instructor_实体集之间的一对一联系集。
    + _student_和_instructor_实体集之间的多对一联系集。
]

#question[
    （_8.6_） 对于模式$R=(A,B,C,D,E)$计算如下函数依赖集$F$的闭包：$ A arrow.r B C\ C D arrow.r E \ B arrow.r D\ E arrow.r A $列出$R$的候选码。Tips：可以使用$*$代表$R$中任意属性集，$alpha$代表${A,B,C,D,E}$任意子集。
]

#question[
    （_8.7_） 利用@1-6-3 的函数依赖计算正则覆盖$F_c$。
]

#question[
    （_8.25_） 使用函数依赖定义论证阿姆斯特朗公理（自反律、增补律与传递律）均有效。
]

#question[
    （_8.26_） 给出一个关系$r$说明如下函数依赖规则不是有效的：$ alpha arrow.r beta and gamma arrow.r beta arrow.r.double alpha arrow.r gamma $。
]

#question[
    （_8.30_） 模式$(A,B,C,D,E,G)$上有函数依赖集$F$：$ A arrow.r B C D\ B C arrow.r D E\ B arrow.r D\ D arrow.r A $
    + 计算$B^+$。
    + 使用阿姆斯特朗公理证明$A G$是超码。
    + 计算$F$的一个正则覆盖。
    + 根据*c*的正则覆盖给出给定模式的3NF分解。
    + 根据$F$对给定模式进行BCNF分解。
]

#question[
    （_8.31_） 模式$R=(A,B,C,D,E,G)$上有函数依赖集$F$：$ A B arrow.r C D\ B arrow.r D\ D E arrow.r B\ D E G arrow.r A B\ A C arrow.r D E $
    + 解释为何$A B arrow.r C D$表明$R in.not "BCNF"$。
    + 从$A B arrow.r C D$开始生成$R$的BCNF分解并确定结果是否是保持依赖的。
]

#question[
    （_8.32_） 模式$R=(A,B,C,D,E,G)$上有函数依赖集$F$：$ A arrow.r B C\ B D arrow.r E\ C D arrow.r A B $
    + 给出一个被上述依赖所逻辑蕴含的不包含无关属性的非平凡函数依赖。
    + 给出从$A arrow.r B C$开始的对$R$的BCNF分解。
    + 说明*b*是否是无损的。
    + 说明*b*是否是保持依赖的。
]

#question[
    （_8.33_） 模式$R=(A,B,C,D,E,G)$上有函数依赖集$F$：$ A B arrow.r C D\ A D E arrow.r G D E\ B arrow.r G C\ G arrow.r D E $给出$R$的3NF分解：
    + 所有候选码。
    + $F$的一个正则覆盖。
    + 完整算法。
    + 分解。
...
#question[
    （_8.34_） 模式$R=(A,B,C,D,E,G,H)$上有函数依赖集$F$：$ A B arrow.r C D\ D arrow.r C\ D E arrow.r B\ D E H arrow.r A B\ A C arrow.r D C $给出$R$的3NF分解：
    + 所有候选码。
    + $F$的一个正则覆盖。
    + 完整算法。
    + 分解。
